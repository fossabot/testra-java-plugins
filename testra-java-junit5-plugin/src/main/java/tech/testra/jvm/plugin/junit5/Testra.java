...
public class Testra implements TestExecutionListener {
  ...
  public void executionStarted(TestIdentifier testIdentifier) {
    ...
    commonData.startTime = System.currentTimeMillis();
      ...
      final Optional<MethodSource> methodSource =
              testIdentifier
                      .getSource()
    ...
    testcaseRequest.setName(testIdentifier.getDisplayName());
      methodSource.ifPresent(
              source -> {
                  String namespace =
                          source.getClassName().substring(0, source.getClassName().lastIndexOf("."));
                  String classname =
                          source.getClassName().substring(source.getClassName().lastIndexOf(".") + 1);
      ...
      List<String> tagList =
              testIdentifier.getTags().stream().map(x -> x.getName()).collect(Collectors.toList());
  ...
  public void executionFinished(
    ...
    TestResultRequest testResultRequest = new TestResultRequest();
      ...
      case FAILED:
        testResultRequest.setStatus(StatusEnum.FAILED);
          testResultRequest.setError(
                  testExecutionResult.getThrowable().map(Throwable::toString).orElse(null));
        break;
      case ABORTED:
        testResultRequest.setStatus(StatusEnum.FAILED);
          testResultRequest.setError(
    ...
    testResultRequest.setRetryCount(0);
      ...
      commonData.resultCounter = 0;
          TestraRestClient.updateResult(
                  failedScenarioIDs.get(commonData.currentTestCaseID), testResultRequest);
      } else TestraRestClient.createResult(testResultRequest);
  }
